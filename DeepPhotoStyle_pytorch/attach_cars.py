...
def attach_car_to_scene_fixed(scene_img, adv_car_img, car_img, car_mask, object_name):
    ...
    for idx_Bat in range(B_Sce):
        ...
        trans_seq = transforms.Compose([transforms.Resize([int(H * scale), int(W * scale)])])
        ...
        adv_scene[idx_Bat, :, h_range, w_range] = adv_car_img_trans * car_mask_trans + car_area_in_scene * (1 - car_mask_trans)
        car_scene[idx_Bat, :, h_range, w_range] = car_img_trans * car_mask_trans + car_area_in_scene * (1 - car_mask_trans)
...
def attach_car_to_scene(scene_img, adv_car_img, car_img, car_mask, batch_size, paint_mask, object_name):
    ...
    for idx_Bat in range(B_Sce):
        ...
        # Do some transformation on the adv_car_img together with car_mask
        trans_seq = transforms.Compose([transforms.RandomRotation(degrees=3), transforms.Resize([int(H * scale), int(W * scale)])])
