...
mapfile -t COLOR_SWEEP < <(python - <<'PY'
import numpy as np
vals = np.geomspace(5, 60, 30)
for v in vals:
    print(f"{v:.8g}")
PY
)
...
EXPERIMENT_TAG="5_bmw_cw14_cw2356_log30_tvw0_midu50_adam"
...
for i in "${!COLOR_SWEEP[@]}"; do
    cw_value="${COLOR_SWEEP[$i]}"
    RESULT_FILE="$LOG_DIR/${i}_${LABEL_PREFIX}_cw14_${cw_value}_cw2356_${cw_value}_tv_0_adam.txt"
    ...
    echo "启动实验 $i，cw14=cw2356=$cw_value，日志输出到 $RESULT_FILE" >> "$LOG_FILE"
    wait
