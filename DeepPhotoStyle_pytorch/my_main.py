...
if __name__ == "__main__":
    ...
    output, yolo_model = run_style_transfer(
        logger,
        cnn,
    ...
    # np.save(os.path.join(log_dir, 'eval_car_scene_disp.npy'), car_scene_disip)
    ...
    car_scene_disp = yolo_model(tensor_to_image(car_scene_out))
    adv_scene_disp = yolo_model(tensor_to_image(adv_scene_out))
    scene_disp = yolo_model(tensor_to_image(test_scene_img))

    df = adv_scene_disp.pandas().xyxy[0]
    for index, row in df.iterrows():
        print(f"检测到: {row['name']} (ID={row['class']}), " f"置信度: {row['confidence']:.2f}, " f"位置: [{row['xmin']:.0f}, {row['ymin']:.0f}, {row['xmax']:.0f}, {row['ymax']:.0f}]")

    cv2.imwrite("./test_output/eval_scene_disp.jpg", cv2.cvtColor(scene_disp.render()[0], cv2.COLOR_RGB2BGR))
    cv2.imwrite("./test_output/eval_car_scene_disp.jpg", cv2.cvtColor(car_scene_disp.render()[0], cv2.COLOR_RGB2BGR))
    cv2.imwrite("./test_output/eval_adv_scene_disp.jpg", cv2.cvtColor(adv_scene_disp.render()[0], cv2.COLOR_RGB2BGR))
