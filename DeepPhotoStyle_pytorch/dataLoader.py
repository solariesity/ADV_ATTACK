...
def pil_loader(path, rgb=True):
    try:
        with open(path, "rb") as f:
            img = Image.open(f)
            return img.convert("RGB") if rgb else img.convert("I")
    except:
        return None  # 遇到任何错误直接静默跳过
...
class KittiLoader(Dataset):
    ...
    def __init__(
        self,
        root_dir=utils.kitti_object_path,
        mode="train",
        loader=pil_loader,
        size=(1024, 320),
        train_list="vehicle_detection/training.txt",
        val_list="vehicle_detection/testing.txt",
        data_limit: int = -1,
    ):
    ...
    def get_depth(self, depth_path):
        ...
        sparse_depth = np.asarray(sparse_depth) / 256.0
    ...
    def __getitem__(self, idx):
        color_path, label = self.filepaths[idx]
        color = self.get_color(color_path)
        if color is None:  # 跳过损坏文件，递归加载下一个样本
